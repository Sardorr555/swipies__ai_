...
class Base(ABC):
    def __init__(self, key, model_name, base_url, **kwargs):
        ...
        self.base_url = ensure_v1(base_url) if base_url else None
        try:
            import httpx
            http_client = httpx.Client(timeout=timeout)
            async_http_client = httpx.AsyncClient(timeout=timeout)
            self.client = OpenAI(api_key=key or "dummy", base_url=self.base_url, http_client=http_client)
            self.async_client = AsyncOpenAI(api_key=key or "dummy", base_url=self.base_url, http_client=async_http_client)
        except Exception:
            self.client = OpenAI(api_key=key or "dummy", base_url=self.base_url, timeout=timeout)
            self.async_client = AsyncOpenAI(api_key=key or "dummy", base_url=self.base_url, timeout=timeout)
