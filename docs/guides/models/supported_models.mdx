...
A complete list of model providers supported by Swipies, which will continue to expand.
...
You can use **AI Badgr** with Swipies via the existing OpenAI-API-Compatible provider.
...
AI Badgr implements OpenAI-compatible endpoints for `/v1/chat/completions`, `/v1/embeddings`, and `/v1/models`, so no additional code changes in Swipies are required.

:::note
The list of supported models is extracted from [this source](https://github.com/Sardorr555/swipies__ai_/blob/main/rag/llm/__init__.py) and may not be the most current. For the latest supported model list, please refer to the Python file.
:::
