...
Swipies supports deploying models locally using Ollama, Xinference, IPEX-LLM, vLLM ，SGLang , GPUStack or jina. If you have locally deployed models to leverage or wish to enable GPU or CUDA for inference acceleration, you can bind Ollama or Xinference into Swipies and use either of them as a local "server" for interacting with your local models.

Swipies seamlessly integrates with Ollama and Xinference, without the need for further environment configurations. You can use them to deploy two types of local models in Swipies: chat models and embedding models.

:::tip NOTE
This user guide does not intend to cover much of the installation or configuration details of Ollama or Xinference; its focus is on configurations inside Swipies. For the most current information, you may need to check out the official site of Ollama or Xinference.
:::
...
- If Swipies runs in Docker, the localhost is mapped within the Swipies Docker container as `host.docker.internal`. If Ollama runs on the same host machine, the right URL to use for Ollama would be `http://host.docker.internal:11434/' and you should check that Ollama is accessible from inside the Swipies container with:
```bash
$ sudo docker exec -it docker-swipies-cpu-1 bash
$ curl http://host.docker.internal:11434/
> Ollama is running
```

- If Swipies is launched from source code and Ollama runs on the same host machine as Swipies, check if Ollama is accessible from Swipies's host machine:
```bash
$ curl http://localhost:11434/
> Ollama is running
```

- If Swipies and Ollama run on different machines, check if Ollama is accessible from Swipies's host machine:
```bash
...
In Swipies, click on your logo on the top right of the page **>** **Model providers** and add Ollama to Swipies:

![add ollama](https://github.com/Sardorr555/swipies__ai_/assets/93570324/10635088-028b-4b3d-add9-5c5a6e626814)
...
In Swipies, click on your logo on the top right of the page **>** **Model providers** and add Xinference to Swipies:

![add xinference](https://github.com/Sardorr555/swipies__ai_/assets/93570324/10635088-028b-4b3d-add9-5c5a6e626814)
...
To enable IPEX-LLM accelerated Ollama in Swipies, you must also complete the configurations in Swipies. The steps are identical to those outlined in the *Deploy a local model using Ollama* section:
...
```
### 5.2 INTERGRATEING SWPIES WITH VLLM CHAT/EM/RERANK LLM WITH WEBUI
...
```
### 6.2 INTERGRATEING SWPIES WITH GPUSTACK CHAT/EM/RERANK LLM WITH WEBUI
