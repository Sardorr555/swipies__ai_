...
Swipies's `CodeExec` agent component needs a sandbox provider to run Python and JavaScript code.

The simplest setup flow is:

1. Start the required sandbox services.
2. Open the Swipies admin page.
...
Swipies supports multiple sandbox providers. Configure the active provider in
...
- Docker Compose version 2.26.1 or higher (similar to Swipies requirements).
...
docker tag swipies/sandbox-base-python:latest sandbox-base-python:latest
docker tag swipies/sandbox-base-nodejs:latest sandbox-base-nodejs:latest
```
...
4. Start the Swipies service as usual.
...
- `SANDBOX_ENABLED`: Enables sandbox support in Swipies.
