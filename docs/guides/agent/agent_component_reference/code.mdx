...
Swipies Sandbox is a secure, pluggable code execution backend. It serves as the code executor for the **Code** component. Please follow the [instructions here](https://github.com/Sardorr555/swipies__ai_/tree/main/agent/sandbox) to install Swipies Sandbox.

:::note Docker client version
The executor manager image now bundles Docker CLI `29.1.0` (API 1.44+). Older images shipped Docker 24.x and will fail against newer Docker daemons with `client version 1.43 is too old`. Pull the latest `swipies/sandbox-executor-manager:latest` or rebuild it in `./sandbox/executor_manager` if you encounter this error.
:::

:::tip NOTE
If your Swipies Sandbox is not working, please be sure to consult the [Troubleshooting](#troubleshooting) section in this document. We assure you that it addresses 99.99% of the issues!
:::
...
### 4. Enable Sandbox-specific settings in Swipies

Ensure all Sandbox-specific settings are enabled in **swipies/docker/.env**.
...
```JavaScript
    ...
    async function main(args) {
      try {
        const response = await axios.get('https://github.com/Sardorr555/swipies__ai_');
...
```bash
docker pull swipies/sandbox-base-nodejs:latest
docker pull swipies/sandbox-base-python:latest
```
...
```bash
docker pull swipies/sandbox-executor-manager:latest
# or
...
```bash {4,6}
(swipies) ➜ swipies/sandbox main ✓ pwd # make sure you are in the right directory
/home/swipies/workspace/swipies/sandbox

(swipies) ➜ swipies/sandbox main ✓ echo "openpyxl" >> sandbox_base_image/python/requirements.txt # add the package to the requirements.txt file

(swipies) ➜ swipies/sandbox main ✗ cat sandbox_base_image/python/requirements.txt # make sure the package is added
numpy
pandas
requests
openpyxl # here it is

(swipies) ➜ swipies/sandbox main ✗ make # rebuild the docker image, this command will rebuild the image and start the service immediately. To build image only, using `make build` instead.

(swipies) ➜ swipies/sandbox main ✗ docker exec -it sandbox_python_0 /bin/bash # entering container to check if the package is installed
...
```bash
(swipies) ➜ swipies/sandbox main ✓ pwd
/home/swipies/workspace/swipies/sandbox

(swipies) ➜ swipies/sandbox main ✓ cd sandbox_base_image/nodejs

(swipies) ➜ swipies/sandbox/sandbox_base_image/nodejs main ✓ npm install lodash 

(swipies) ➜ swipies/sandbox/sandbox_base_image/nodejs main ✓ cd ../.. # go back to sandbox root directory

(swipies) ➜ swipies/sandbox main ✗ make # rebuild the docker image, this command will rebuild the image and start the service immediately. To build image only, using `make build` instead.

(swipies) ➜ swipies/sandbox main ✗ docker exec -it sandbox_nodejs_0 /bin/bash # entering container to check if the package is installed
