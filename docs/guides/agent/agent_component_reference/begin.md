...
No. Files uploaded to an agent as input are not stored in a dataset and hence will not be processed using Swipies's built-in OCR, DLR or TSR models, or chunked using Swipies's built-in chunking methods.
...
:::tip NOTE
The variables `MAX_CONTENT_LENGTH` in `/docker/.env` and `client_max_body_size` in `/docker/nginx/nginx.conf` set the file size limit for each upload to a dataset or Swipies's File system. These settings DO NOT apply in this scenario.
:::
