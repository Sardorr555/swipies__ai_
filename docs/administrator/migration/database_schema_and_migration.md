...
The [mysql_migration.py](https://github.com/Sardorr555/swipies__ai_/blob/main/tools/scripts/mysql_migration.py) script is a specialized tool for re-organizing Swipies’s model-related data. It transitions data from older unified tables into a modern, multi-table structure to support advanced model management.
...
- **Version upgrades**: Essential when moving to Swipies v0.25 or later to ensure your models are correctly categorized in the new schema.
...
The [db_schema_sync.py](https://github.com/Sardorr555/swipies__ai_/blob/main/tools/scripts/db_schema_sync.py) script is a synchronization utility that ensures your MySQL database structure matches the Peewee ORM models defined in the Swipies source code.
...
- **Version upgrades**: When moving to a new version of Swipies that introduces structural database changes.
