...
A common scenario is processing large datasets on a powerful instance (e.g., with a GPU) and then migrating the entire Swipies service to a different production environment (e.g., a CPU-only server). This guide explains how to safely back up and restore your data using our provided migration script.

### Identify your data

By default, Swipies uses Docker volumes to store all persistent data, including your database, uploaded files, and search indexes. You can see these volumes by running:
...
:::note
The volume name prefix (e.g., `docker_`) comes from the Docker Compose project name. By default it is `docker` (derived from the directory name). If you started Swipies with `docker compose -p <project_name>`, your volumes will be prefixed with `<project_name>_` instead, for example `swipies_mysql_data`.
:::

### Step 1: Stop Swipies services

Before starting the migration, you must stop all running Swipies services on the **source machine**. Navigate to the project's root directory and run:

```bash
docker compose -f docker/docker-compose.yml down
```

If you started Swipies with a custom project name (e.g., `docker compose -p swipies`), include it in the command:

```bash
docker compose -p swipies -f docker/docker-compose.yml down
```
...
```bash
bash docker/migration.sh backup my_swipies_backup
```

This will create a folder named `my_swipies_backup/` instead.

If you started Swipies with a custom project name (e.g., `docker compose -p swipies`), use the `-p` flag so the script can find the correct volumes:

```bash
bash docker/migration.sh -p swipies backup
bash docker/migration.sh -p swipies backup my_swipies_backup
```

### Step 3: Transfer the backup folder

Copy the entire backup folder (e.g., `backup/` or `my_swipies_backup/`) from your source machine to the Swipies project directory on your **target machine**. You can use tools like `scp`, `rsync`, or a physical drive for the transfer.

### Step 4: Restore your data

On the **target machine**, ensure that Swipies services are not running. Then, use the migration script to restore your data from the backup folder.
...
```bash
bash docker/migration.sh restore my_swipies_backup
```
...
```bash
bash docker/migration.sh -p swipies restore
bash docker/migration.sh -p swipies restore my_swipies_backup
```
...
### Step 5: Start Swipies services

Once the restore process is complete, you can start the Swipies services on your new machine:
...
```bash
docker compose -p swipies -f docker/docker-compose.yml up -d
```
...
Your Swipies instance is now running with all the data from your original machine.
...
By default, Swipies creates one bucket per Knowledge Base (dataset) and one bucket per user folder. This can be problematic when:
...
The **Single Bucket Mode** allows you to configure Swipies to use a single bucket with a directory structure instead of multiple buckets.
...
minio:
  ...
  bucket: "swipies-bucket" # Default bucket name
  prefix_path: "swipies" # Optional prefix path
```
...
export MINIO_BUCKET=swipies-bucket
export MINIO_PREFIX_PATH=swipies
```
...
[Tigris](https://www.tigrisdata.com) is an S3-compatible object storage service that works with Swipies's `AWS_S3` backend. Set `STORAGE_IMPL=AWS_S3` in your `.env` file:
...
mc ls old-minio/ | grep kb_ | while read -r line; do
    ...
    mc cp --recursive old-minio/$bucket/ new-minio/swipies-bucket/swipies/$bucket/
done
```
