...
def generate_license(owner: str, expiry: str, lic_type: str, **kwargs) -> str:
    # Ensure every generated key is globally unique even if owner, expiry, and type are identical
    unique_nonce = kwargs.pop("nonce", None) or kwargs.pop("id", None) or uuid.uuid4().hex[:12]
    payload = {
        "owner": owner,
        "expiry": expiry,
        "type": lic_type,
        "nonce": str(unique_nonce)
    }
