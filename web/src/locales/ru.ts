...
[Вставьте текст здесь]`,
    ...
    privacyPolicy: {
      back: 'Назад',
      title: 'Политика конфиденциальности и SLA обработки данных',
      effectiveDate: 'Дата последнего обновления',
      slaBadge1: '99.9% SLA Доступности',
      slaBadge2: 'Шифрование AES-256',
      slaBadge3: 'Изоляция арендаторов',
      slaBadge4: 'Без обучения AI',
      section1Title: '1. Общие положения и соглашение SLA',
      section1Text:
        'Настоящий документ регламентирует правила обработки, хранения, защиты и удаления данных пользователей на платформе Swipies («мы», «наш», «платформа»), а также определяет параметры качества и надежности сервиса (SLA — Service Level Agreement). Пользуясь платформой, вы безоговорочно принимаете условия данного соглашения.',
      section2Title: '2. Категории и типы собираемых данных',
      section2Text:
        'Мы собираем и обрабатываем исключительно те данные, которые необходимы для функционирования нейросетевых сервисов, поиска по знаниям и работы агентов:',
      section2Item1:
        'Данные профиля: имя (никнейм), адрес электронной почты, номер телефона, хэш пароля, токены авторизации и подписки.',
      section2Item2:
        'Загружаемый контент: документы (PDF, DOCX, TXT, CSV, XLSX, Markdown), изображения, аудиозаписи и файлы баз знаний.',
      section2Item3:
        'Векторные представления (эмбеддинги): сгенерированные числовые векторы и чанки текста, созданные в процессе индексации документов.',
      section2Item4:
        'История диалогов и контекст: промпты, история сообщений, промежуточные шаги агентов (reasoning), метрики и логи вызовов API.',
      section2Item5:
        'Технические метрики: IP-адрес, данные браузера, время отклика, логи использования системных ресурсов.',
      section3Title: '3. Архитектура хранения и шифрование данных',
      section3Text:
        'Хранение данных организовано с соблюдением максимальных стандартов информационной безопасности:',
      section3Item1:
        'Реляционные базы данных: метаданные, настройки, сессии и структура диалогов хранятся в защищенных БД MySQL/PostgreSQL.',
      section3Item2:
        'Объектное хранилище (MinIO/S3): загруженные исходные файлы хранятся в изолированном хранилище с ограниченным контуром доступа.',
      section3Item3:
        'Поисковые и векторные индексы: чанки документов и векторные представления индексируются в защищенных движках (Elasticsearch/Infinity).',
      section3Item4:
        'Шифрование данных: все данные в покое шифруются алгоритмом AES-256. Передача данных между клиентом, сервером и сервисами осуществляется по протоколам TLS 1.3 / HTTPS.',
      section3Item5:
        'Изоляция арендаторов (Tenant Isolation): данные каждого тенанта и пользователя строго изолированы на уровне баз данных и ключей доступа.',
      section4Title: '4. Процесс обработки, чанкинга и RAG-поиска',
      section4Text:
        'При загрузке файлов система выполняет извлечение текста (OCR, parsing), разбиение на смысловые фрагменты (чанки) и построение гибридного индекса. Извлеченные данные используются исключительно для формирования контекста ответов нейросети по вашему запросу.',
      section5Title: '5. Замена конфиденциальных данных (Sensitive Data Replacement)',
      section5Text:
        'Платформа предоставляет встроенный модуль автоматической анонимизации данных:',
      section5Item1:
        'Локальная маскировка: перед отправкой промпта в нейросеть выполняются поиск совпадений по заданным текстовым и регулярным правилам (RegEx) и их замена на анонимные плейсхолдеры.',
      section5Item2:
        'Обратная деанонимизация: при получении ответа от нейросети анонимные плейсхолдеры автоматически восстанавливаются в исходные значения на стороне сервера перед отображением пользователю.',
      section5Item3:
        'Защита от утечек: активируя переключатель замены конфиденциальных данных, вы подтверждаете свое согласие на хранение правил замены и обработку данных в соответствии с настоящим SLA.',
      section6Title: '6. Передача данных третьим лицам и сторонним LLM',
      section6Text:
        'Мы не продаем и не передаем ваши данные коммерческим третьим лицам. Передача данных осуществляется только провайдерам LLM (OpenAI, Anthropic, DeepSeek, Google и др.), выбранным пользователем для генерации ответов. Мы работаем по коммерческим API-соглашениям, гарантирующим, что ваши данные НЕ используются для обучения сторонних моделей ИИ.',
      section7Title: '7. Гарантии доступности и SLA платформы',
      section7Text:
        'Мы гарантируем целевой уровень доступности платформы (Uptime SLA) не менее 99.9% в месяц, за исключением плановых технических работ:',
      section7Item1:
        'Резервное копирование: автоматическое создание ежедневных резервных копий баз данных и объектных хранилищ.',
      section7Item2:
        'Отказоустойчивость: дублирование микросервисных компонентов и автоматически масштабируемая инфраструктура.',
      section7Item3:
        'Мониторинг 24/7: непрерывный мониторинг исправности систем и автоматическое оповещение об инцидентах.',
      section8Title: '8. Сроки хранения и удаление данных',
      section8Text:
        'Ваши данные хранятся на серверах в течение всего времени активности вашей учетной записи. Вы имеете право в любой момент полностью удалить любой чат, базу знаний, загруженный файл или всю учетную запись. При удалении объект физически удаляется из всех баз данных, объектных хранилищ и векторных индексов без возможности восстановления.',
      section9Title: '9. Контроль доступа и аудит безопасности',
      section9Text:
        'Доступ к вашим данным строго разграничен на основе ролевой модели (RBAC). Доступ сотрудников платформы к контенту пользователей закрыт и возможен только в исключительных случаях по письменному запросу пользователя в службу поддержки для решения технических проблем.',
      section10Title: '10. Файлы Cookie и локальное хранилище',
      section10Text:
        'Платформа использует файлы Cookie и LocalStorage для сохранения состояния авторизованного сеанса (JWT), настроек интерфейса, выбранного языка и темы оформления.',
      section11Title: '11. Права пользователей и согласие',
      section11Text:
        'Вы имеете право на доступ к своим данным, их экспорт, изменение и полное удаление. Используя платформу Swipies и активируя сервисы взаимодействия с LLM, вы подтверждаете свое согласие с данной Политикой конфиденциальности и Условиями SLA.',
      section12Title: '12. Контакты службы безопасности и поддержки',
      section12Text:
        'По всем вопросам, связанным с обработкой данных, политикой безопасности или условиями SLA, вы можете связаться с нашей службой поддержки по адресу: {{email}}',
    },
  },
};
