export default {
  translation: {
    ...
    login: {
      loginTitle: 'Добро пожаловать в лучшее AI-приложение в истории 🚀',
      signUpTitle: 'Присоединяйся к самой мощной AI-платформе ✨',
      login: 'Войти',
      signUp: 'Зарегистрироваться',
      loginDescription: 'Мы безумно рады видеть тебя снова! Готов к новым AI-открытиям? 🔥',
      registerDescription: 'Твой первый шаг в будущее искусственного интеллекта начинается здесь! 🌟',
      emailLabel: 'Email',
      ...
      marketingConsentLabel: 'Я согласен(-на) получать рекламную рассылку, новости и специальные предложения',
...
Пример: сообщение 1 КБ с эмбеддингом 1024 измерений — около 9 КБ. Лимит по умолчанию 5 МБ — примерно 500 таких сообщений.`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Проведите тест извлечения, чтобы проверить, может ли Swipies восстановить целевой контент для LLM.',
      Parse: 'Парсинг',
      dataset: 'Датасет',
      ...
      uploadDescription:
        'Поддерживает одиночную или пакетную загрузку файлов. Для локально развернутого Swipies: общий ограничение размера файлов за одну загрузку составляет 1 ГБ, с ограничением пакетной загрузки в 32 файла. Нет ограничения на общее количество файлов на аккаунт. Для cloud.swipies.io общий ограничение размера файлов за одну загрузку составляет 10 МБ, каждый файл не должен превышать 10 МБ, максимум 128 файлов на аккаунт.',
      chunk: 'Чанк',
      bulk: 'Пакетно',
      cancel: 'Отмена',
      close: 'Закрыть',
      ...
      rerankTip: `Опционально. Если оставить пустым, Swipies будет использовать комбинацию взвешенной схожести по ключевым словам и взвешенного косинусного сходства векторов; если выбрана rerank модель, взвешенный reranking счет заменит взвешенное косинусное сходство векторов. Имейте в виду, что использование rerank модели значительно увеличит время отклика системы. Если вы хотите использовать rerank модель, убедитесь, что вы используете SaaS reranker; если вы предпочитаете локально развернутую rerank модель, убедитесь, что вы запускаете Swipies с помощью docker-compose-gpu.yml.`,
      topK: 'Топ-K',
      ...
      html4excelTip: `Используется с методом чанкинга General. Когда отключено, электронные таблицы (XLSX или XLS (Excel 97-2003)) в базе знаний будут разобраны в пары ключ-значение. Когда включено, они будут разобраны в HTML таблицы, разделяя каждые 12 строк, если исходная таблица имеет более 12 строк. Подробности см. на https://docs.swipies.app/docs/dev/enable_excel2html.`,
      autoKeywords: 'Авто-ключевые слова',
      autoKeywordsTip: `Автоматически извлекает N ключевых слов для каждого чанка, чтобы повысить их рейтинг для запросов, содержащих эти ключевые слова. Имейте в виду, что дополнительные токены будут потребляться чат-моделью, указанной в 'Системных настройках модели'. Вы можете проверить или обновить добавленные ключевые слова для чанка из списка чанков. Подробности см. на https://docs.swipies.app/docs/dev/autokeyword_autoquestion.`,
      autoQuestions: 'Авто-вопросы',
      autoQuestionsTip: `Автоматически извлекает N вопросов для каждого чанка, чтобы повысить их рейтинг для запросов, содержащих эти вопросы. Вы можете проверить или обновить добавленные вопросы для чанка из списка чанков. Эта функция не нарушит процесс чанкинга в случае ошибки, за исключением того, что может добавить пустой результат в исходный чанк. Имейте в виду, что дополнительные токены будут потребляться LLM, указанной в 'Системных настройках модели'. Подробности см. на https://docs.swipies.app/docs/dev/autokeyword_autoquestion.`,
...
`,
      useRaptor: 'RAPTOR',
      useRaptorTip:
        'RAPTOR можно использовать для многошаговых вопросно-ответных задач. Перейдите на страницу Файлы, нажмите Сгенерировать > RAPTOR. Подробности: https://docs.swipies.app/docs/dev/enable_raptor',
      prompt: 'Промпт',
      promptTip:
...
Выше приведено содержимое, которое вам нужно суммировать.`,
      ...
      vietnamese: 'Вьетнамский',
      pageRank: 'Page rank',
      pageRankTip: `Вы можете назначить более высокий балл PageRank определённым базам знаний при извлечении. Соответствующий балл добавляется к комбинированным оценкам схожести извлечённых чанков, повышая их рейтинг. Подробности: https://docs.swipies.app/docs/dev/set_page_rank`,
      tagName: 'Тег',
      frequency: 'Частота',
      ...
      tagSet: 'Наборы тегов',
      tagSetTip: `
     <p> Выберите одну или несколько баз тегов для автоматического тегирования чанков. Подробности: https://docs.swipies.app/docs/dev/use_tag_sets</p>
...
<ul>
  ...
  <li>Авто-ключевые слова зависят от LLM и потребляют много токенов.</li>
      ...
      useGraphRagTip:
        'Строит граф знаний поверх чанков файлов текущей базы знаний для многошагового Q&A с вложенной логикой. Подробности: https://docs.swipies.app/docs/dev/construct_knowledge_graph',
...
**Основные правила:**
  ...
  - **Всегда учитывайте** всю историю диалога.`,
      ...
      sensitiveDataStorageWarning: 'Все вводимые правила и данные сохраняются у нас на сервере независимо от состояния переключателя (включен или выключен). Изменяя его положение, вы соглашаетесь с этим.',
      featureDisabledNotice: 'Функция замены сейчас отключена (промпты отправляются в LLM без изменений), но ваши правила сохранены у нас на сервере.',
      ...
      importTemplateDialogTitle: 'Формат файла и примеры для импорта правил',
      importTemplateDialogDesc: 'Вы можете импортировать правила замены из файлов формата JSON или CSV. Скачайте готовый пример ниже или оформите ваш файл в соответствии с образцом.',
      downloadJsonTemplate: 'Скачать пример JSON',
      downloadCsvTemplate: 'Скачать пример CSV',
      searchValue: 'Что искать',
      replaceValue: 'На что заменить',
      caseSensitive: 'Регистр',
      regex: 'RegEx',
      searchPlaceholder: 'например, Иван Иванов',
      replacePlaceholder: 'например, CLIENT_NAME',
      model: 'Модель',
      ...
      keyNamePlaceholder: 'Введите название ключа...',
      created: 'Создано',
      action: 'Действие',
      ...
      betaError:
        'Пожалуйста, сначала получите Swipies API ключ на странице Системных настроек.',
      searching: 'Поиск...',
      parsing: 'Парсинг',
      uploading: 'Загрузка',
...
Пример: Virtual Hosted Style`,
      ...
      deleteAccountDescription: 'После удаления ваш аккаунт нельзя будет восстановить вообще.',
      deleteAccountBtn: 'Удалить аккаунт',
      deleteAccountConfirmTitle: 'Серьёзно? Удалить аккаунт? 😢',
      deleteAccountConfirmContent: 'Ты и вправду хочешь покинуть лучшее AI-приложение в истории? Все твои настройки, диалоги и персональные агенты будут безвозвратно удалены. Мы будем очень скучать по тебе! 💔',
      logoutConfirmTitle: 'Выход из системы 🚀',
      logoutConfirmContent: 'Ты и вправду хочешь выйти из самого крутого приложения в истории? ✨',
      logoutConfirmOk: 'Да, хочу выйти',
      logoutConfirmCancel: 'Остаться',
      maxTokens: 'Макс. Токенов',
      ...
      referralDescription:
        'Приглашайте друзей зарегистрироваться и получайте бонусы: +1 ГБ диска и +5 агентов/приложений за каждого приглашенного пользователя!',
      ...
      storageBonus: 'Бонус к памяти',
      agentsBonus: 'Бонус к агентам',
      noReferralsYet:
        'У вас пока нет рефералов. Поделитесь ссылкой, чтобы пригласить друзей!',
      ...
      embeddingModelTip:
        'Модель эмбеддингов по умолчанию для каждой новой созданной базы знаний. Если вы не можете найти модель эмбеддингов в выпадающем списке, проверьте, используете ли вы облегченную версию Swipies (которая не включает модели эмбеддингов) или проверьте https://docs.swipies.app/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
      img2txtModel: 'VLM',
      img2txtModelTip:
        'VLM по умолчанию для каждой новой созданной базы знаний. Она описывает изображение или видео. Если вы не можете найти модель в выпадающем списке, проверьте https://docs.swipies.app/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
      ...
      rerankModel: 'Rerank',
      rerankModelTip: `Модель rerank по умолчанию для переранжирования чанков. Если вы не можете найти модель в выпадающем списке, проверьте https://docs.swipies.app/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.`,
      ttsModel: 'TTS',
      ttsModelTip:
        'Модель преобразования текста в речь по умолчанию. Если вы не можете найти модель в выпадающем списке, проверьте https://docs.swipies.app/docs/dev/supported_models, чтобы узнать, поддерживает ли ваш провайдер моделей эту модель.',
...
[Вставьте текст здесь]`,
    ...
    admin: {
      ...
      status: 'Статус',
      id: 'ID',
      referrals: 'Приглашенные (рефералы)',
      referredBy: 'Приглашен кем',
      referredById: 'ID пригласившего',
    ...
    privacyPolicy: {
      back: 'Назад',
      title: 'Соглашение об уровне обслуживания (SLA) и Политика обработки данных',
      effectiveDate: 'Дата вступления в силу',
      quickNav: 'Быстрый переход:',
      quickNavData: 'Сбор данных',
      quickNavSecurity: 'Безопасность',
      quickNavAtmos: 'Оплата Atmos',
      quickNavRefund: 'Возврат средств',
      quickNavContacts: 'Контакты',
      scrollToTop: 'Наверх',
      trustBoxTitle: 'Платежный шлюз Atmos (atmos.uz) • Стандарт PCI DSS Level 1',
      trustBoxDesc: 'Все онлайн-платежи защищены криптографическим шифрованием TLS/SSL 256-bit. Реквизиты банковских карт вводятся в защищенном контуре шлюза Atmos и не сохраняются серверами Swipies.',
      slaBadge1: '99.9% SLA Аптайма',
      slaBadge2: 'Шифрование AES-256',
      slaBadge3: 'Изоляция арендаторов',
      slaBadge4: 'Без обучения ИИ',
      section1Title: '1. Общие положения и область действия SLA',
      section1Text:
        'Настоящее Соглашение об уровне обслуживания (SLA) и Политика обработки данных регламентируют порядок сбора, хранения, защиты, анонимизации, использования и физического удаления данных на платформе Swipies («мы», «наш», «платформа»), а также устанавливает юридические и технические гарантии доступности сервисов. Использование любых компонентов платформы, API или субагентов означает ваше полное согласие со всеми условиями данного SLA.',
      section2Title: '2. Категории и исчерпывающий перечень собираемых данных',
      section2Text:
        'Платформа собирает и обрабатывает исключительно те данные, которые необходимы для аутентификации, генерации нейросетевых ответов, работы Retrieval-Augmented Generation (RAG) и оркестрации агентов:',
      section2Item1:
        'Данные учетной записи: ФИО/никнейм, адрес электронной почты, номер телефона, хэшированный пароль (argon2/bcrypt), маркеры авторизации JWT и параметры подписки.',
      section2Item2:
        'Загружаемый контент пользователей: файлы документов (PDF, DOCX, TXT, CSV, XLSX, Markdown, HTML), изображения, аудиозаписи, таблицы и метаданные загружаемых объектов.',
      section2Item3:
        'Индексы и векторные представления: сгенерированные текстовые фрагменты (чанки) и их многомерные векторные эмбеддинги, создаваемые в процессе индексации знаний.',
      section2Item4:
        'Контекст диалогов и промпты: история запросов, системные промпты, промежуточные шаги рассуждений агентов (thought/reasoning logs), вызовы инструментов (tool calls) и ответы нейросетей.',
      section2Item5:
        'Служебные и технические метрики: IP-адреса, технические логи веб-серверов, токены сессий, временные метки, latency откликов моделей и параметры браузеров.',
      section3Title: '3. Архитектура хранения данных и параметры шифрования',
      section3Text:
        'Хранилища данных Swipies спроектированы в соответствии с жесткими корпоративными стандартами безопасности информационной инфраструктуры:',
      section3Item1:
        'Реляционные базы данных (MySQL / PostgreSQL): хранение учетных записей, настроек ассистентов, связей сессий и метаданных с разделением прав доступа.',
      section3Item2:
        'Объектное хранилище (MinIO / AWS S3): физическое хранение загруженных исходных файлов пользователей в изолированных контейнерах с защищенным периметром.',
      section3Item3:
        'Векторные и поисковые движки (Elasticsearch / Infinity): изолированное хранение текста чанков и векторов для гипертекстового и семантического поиска.',
      section3Item4:
        'Шифрование данных (Encryption SLA): все данные в покое (Data at Rest) шифруются с использованием стойкого алгоритма AES-256. Передача данных по сети (Data in Transit) защищена протоколами TLS 1.3 / HTTPS с поддержкой Perfect Forward Secrecy.',
      section3Item5:
        'Мультиарендная изоляция (Multi-Tenant Isolation): данные каждого клиента и арендатора изолированы на уровне баз данных и криптографических ключей, что полностью исключает возможность межклиентской утечки.',
      section4Title: '4. Процесс обработки, чанкинга и гибридного RAG-поиска',
      section4Text:
        'При загрузке файлов система выполняет асинхронное извлечение текста (OCR, структура документа), разбиение на семантические фрагменты (чанки) и генерацию векторов. Извлеченный контент используется исключительно для построения контекста в момент отправки запроса к нейросети и не используется для иных сторонних задач.',
      section5Title: '5. Модуль автоматической замены конфиденциальных данных (Sensitive Data Replacement)',
      section5Text:
        'Для защиты персональной и коммерческой информации платформой реализован встроенный механизм локальной анонимизации (SDR):',
      section5Item1:
        'Локальная маскировка: перед отправкой промпта во внешнюю LLM система ищет совпадения по настраиваемым текстовым и регулярным правилам (RegEx, с учетом регистра и чисел) и заменяет их на анонимные плейсхолдеры.',
      section5Item2:
        'Обратная деанонимизация в потоке: при получении потокового ответа от нейросети анонимные плейсхолдеры автоматически восстанавливаются в исходные значения на сервере перед выдачей пользователю.',
      section5Item3:
        'Гарантия неразглашения: при включении тумблера Sensitive Data Replacement вы даете явное согласие на обработку правил замены в рамках настоящей Политики SLA.',
      section6Title: '6. Взаимодействие со сторонними LLM и отказ от обучения нейросетей',
      section6Text:
        'Мы не продаем, не передаем и не сдаем в аренду данные пользователей третьим лицам. Передача данных происходит исключительно сторонним поставщикам моделей (OpenAI, Anthropic, DeepSeek, Google, Cohere), выбранным самим пользователем:',
      section6Item1:
        'Запрет обучения ИИ (Zero AI Model Training): коммерческие соглашения с провайдерами гарантируют, что ваши промпты и документы НЕ используются для обучения или дообучения публичных базовых моделей ИИ.',
      section6Item2:
        'Защищенные API-каналы: вызов API внешних LLM выполняется по зашифрованным соединениям от имени платформы без сохранения пользовательских данных у сторонних провайдеров дольше срока обработки запроса.',
      section7Title: '7. Гарантии доступности сервисов (Uptime SLA 99.9%)',
      section7Text:
        'Платформа Swipies гарантирует целевой ежемесячный коэффициент доступности сервиса (Uptime SLA) на уровне не менее 99.9%:',
      section7Item1:
        'Избыточность и отказоустойчивость: дублирование микросервисов, балансировка нагрузки и автоматическое масштабирование при пиковых нагрузках.',
      section7Item2:
        'Резервное копирование (RPO/RTO): автоматическое ежедневное резервное копирование БД и объектных хранилищ с глубиной хранения архива до 30 дней.',
      section7Item3:
        'Круглосуточный мониторинг: автоматический мониторинг доступности узлов 24/7/365 и мгновенное уведомление инженеров платформы.',
      section8Title: '8. Условия онлайн-оплаты и безопасность платежей (Шлюз Atmos)',
      section8Text:
        'Все безналичные платежи на платформе Swipies AI обрабатываются сертифицированным оператором ООО «ATMOS PAYMENT SERVICES» (платежный сервис Atmos, сайт: atmos.uz) в строгом соответствии с законодательством Республики Узбекистан и международными стандартами индустрии платежных карт.',
      section8Item1:
        'Поддерживаемые способы оплаты: К оплате принимаются банковские карты национальных систем Uzcard и HUMO, а также международные карты Visa и MasterCard, выпущенные банками Республики Узбекистан и зарубежными банками.',
      section8Item2:
        'Стандарты безопасности и соответствие PCI DSS Level 1: Передача платежных данных осуществляется по защищенному протоколу с использованием криптографического шифрования TLS 1.2/1.3 (SSL 256-bit). Платежный шлюз Atmos сертифицирован по наивысшему уровню стандарта безопасности индустрии платежных карт PCI DSS Level 1. Платформа Swipies не собирает, не хранит и не обрабатывает конфиденциальные реквизиты ваших банковских карт (полный номер карты, срок действия и CVC/CVV-коды). Все платежные данные вводятся непосредственно в защищенном контуре платежного провайдера.',
      section8Item3:
        'Двухфакторная защита (3-D Secure и OTP): Каждая транзакция проходит обязательную авторизацию через одноразовый SMS-код (OTP), направляемый банком-эмитентом на телефон держателя карты, либо с помощью технологии 3-D Secure (Verified by Visa, Mastercard Identity Check). Списание средств без ведома и подтверждения держателя карты исключено.',
      section8Item4:
        'Валюта расчетов и прозрачность цен: Все расчеты на сайте производятся в национальной валюте Республики Узбекистан — UZS (узбекский сум). Стоимость тарифов и лицензий фиксируется при оформлении заказа и не включает скрытых комиссий. При оплате картами, выпущенными в иностранной валюте, конвертация осуществляется банком-эмитентом карты по его внутреннему курсу.',
      section9Title: '9. Предоставление цифровых услуг и регламент возврата средств',
      section9Text:
        'Все тарифные планы, пакеты AI-токенов и лицензионные ключи Self-Hosted Enterprise, реализуемые на платформе Swipies AI, являются цифровым программным обеспечением (Digital Software Goods).',
      section9Item1:
        'Моментальная цифровая доставка: Активация подписки, зачисление токенов или генерация криптографического лицензионного ключа производятся автоматически в реальном времени сразу после успешного подтверждения списания шлюзом Atmos. Лицензионный ключ и конфигурация моментально отображаются в личном кабинете пользователя.',
      section9Item2:
        'Политика отмены и возврата (Refund Policy): Если услуга не была оказана по техническим причинам, произошел сбой транзакции или двойное списание средств, Пользователь имеет право обратиться в службу поддержки Swipies AI по адресу support@swipies.app или legal@swipies.app с указанием деталей платежа.',
      section9Item3:
        'Возврат исключительно на карту плательщика: В соответствии с правилами банковского эквайринга и процессинга Atmos, возврат денежных средств осуществляется тем же способом и исключительно на ту же банковскую карту, с которой была произведена первоначальная оплата. Срок зачисления возврата составляет от 3 до 14 банковских дней в зависимости от регламента банка-эмитента карты.',
      section10Title: '10. Изменения в данной политике',
      section10Text:
        'Мы можем время от времени обновлять настоящую Политику конфиденциальности. Любые изменения будут опубликованы на этой странице с обновлённой датой вступления в силу. Мы рекомендуем периодически просматривать эту политику. Продолжение использования наших услуг после внесения изменений означает ваше согласие с обновлённой политикой.',
      section11Title: '11. Свяжитесь с нами и реквизиты',
      section11Text:
        'Если у вас есть вопросы или пожелания относительно настоящей Политики конфиденциальности, безопасности данных или условий оплаты через шлюз Atmos, свяжитесь с нами по электронной почте: legal@swipies.app или support@swipies.app',
    },
    license: {
      title: 'Лицензионные ключи (Self-Hosted)',
      refresh: 'Обновить',
      description:
      ...
      envCopied: 'Переменная для docker/.env скопирована!',
      keyDownloaded: 'Лицензионный файл .key скачан!',
      renameSuccess: 'Имя лицензии обновлено',
      renameError: 'Не удалось обновить имя лицензии',
      revokeConfirm: 'Вы уверены, что хотите отозвать данный лицензионный ключ?',
      revokeSuccess: 'Лицензионный ключ отозван',
      revokeError: 'Не удалось отозвать лицензию',
      commandCopied: 'Команда скопирована',
      activeLicensesTitle: 'Активные лицензии',
      oneKeyActive: '1 ключ активен',
      keysActive: '{{count}} ключа(ей) активно',
      zeroKeys: '0 ключей',
      localClusterDesc: 'Локальный защищенный кластер',
      readyToDeploy: 'Готовы к развертыванию на узлах',
      editionTypeTitle: 'Тип редакции',
      selfHosted: 'Self-Hosted',
      enterprise: 'Enterprise',
      dataIsolationDesc: 'Локальная изоляция данных',
      unlimitedDocsLlm: 'Безлимитные документы и LLM',
      securityTitle: 'Безопасность',
      rsa2048: 'RSA-2048',
      signature: 'Signature',
      offlineWorkDesc: 'Абсолютно автономная работа',
      noExternalDeps: 'Без привязки к внешним серверам',
      purchasedTitle: 'Купленные лицензионные ключи',
      totalKeys: 'Всего ключей:',
      loading: 'Загрузка лицензионных ключей...',
      noKeysTitle: 'У вас пока нет лицензионных ключей',
      noKeysDesc:
        'Выберите период подписки ниже, чтобы мгновенно получить криптографический ключ для автономного сервера Swipies AI.',
      choosePlanBelow: 'Выбрать тариф ниже',
      renamePlaceholder: 'Имя сервера/лицензии',
      save: 'Сохранить',
      cancel: 'Отмена',
      ...
      rsaKey: 'RSA Лицензионный ключ',
      hide: 'Скрыть',
      showFullKey: 'Показать полный ключ',
      generatingKey: 'Генерация ключа...',
      waitingGeneration: 'Ожидание генерации...',
      copied: 'Скопировано',
      copy: 'Копировать',
      downloadKey: 'Скачать файл ключа',
      forEnv: 'Для .env:',
      copiedToClipboard: 'Скопировано в буфер',
      copyEnvSnippet: 'Скопировать строку для .env',
      expires: 'Истекает:',
      permanent: 'Бессрочно',
      expired: 'Срок действия истек',
      daysLeft: 'Осталось {{count}} дн.',
      guideTitle: 'Инструкция по активации на вашем сервере',
      guideSubtitle: 'Как применить ключ в Docker Compose за 1 минуту',
      step1Title: '1. Добавьте ключ в конфигурацию Docker:',
      step1Comment: '# В файле docker/.env вашего репозитория добавьте:',
      yourKeyPlaceholder: '<ВАШ_СКОПИРОВАННЫЙ_КЛЮЧ>',
      step2Title: '2. Перезапустите контейнеры приложения:',
      ...
      purchaseTitle: 'Приобрести новый лицензионный ключ',
      purchaseSubtitle:
        'Выберите подходящий период действия. Лицензионный ключ генерируется мгновенно после подтверждения оплаты картой.',
      tier6Title: 'Пилот / 6 Месяцев',
      oneServer: '1 сервер',
      oneTime6Mo: 'Единоразовая оплата на полгода',
      tier6Feat1: 'Безлимитное количество документов и баз знаний',
      tier6Feat2: 'Полная автономная работа без интернета (Air-gap)',
      tier6Feat3: 'Развертывание в локальном Docker окружении',
      tier6Feat4: 'Поддержка через тикеты и документацию',
      buy6Months: 'Купить на 6 месяцев',
      ribbon2MonthsFree: '2 Месяца бесплатно',
      tier12Title: 'Годовая / 12 Месяцев',
      bestPrice: 'Лучшая цена',
      savings100k: 'Экономия 100,000 UZS по сравнению с полугодовой',
      tier12Feat1: 'Все преимущества тарифа на 6 месяцев',
      tier12Feat2: 'Приоритетная линия технической поддержки',
      tier12Feat3: 'Гарантия совместимости будущих обновлений',
      tier12Feat4: 'Поддержка промышленных кластеров Kubernetes',
      buy12Months: 'Купить на 12 месяцев (Выгодно)',
    },
  },
};
