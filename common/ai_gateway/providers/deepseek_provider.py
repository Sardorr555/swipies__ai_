...
logger = logging.getLogger("ai_gateway.deepseek")


class DeepSeekProvider(AIProvider):
    """
    Standardized DeepSeek adapter for AI Gateway.
    Supports deepseek-chat (V3) and deepseek-reasoner (R1) with native thought stream extraction.
    """
    ...
    async def chat_stream(self, request: GatewayChatRequest) -> AsyncIterator[GatewayStreamChunk]:
        try:
            ...
            async for chunk in stream_resp:
                chunk_id = chunk.id or "deepseek_chunk"
                usage = None
