...
async def verify_license():
    """
    Verify a license key generated by the main platform.
    Used by the licensed branch (licence_v) online checks.
    """
    ...
    try:
        ...
        # Query by license key
        licenses = LicenseKeyService.query(license_key=license_key.strip())
        if not licenses:
            return jsonify({"valid": False, "message": "License not found"}), 200

        lic = licenses[0]
        if lic.status == "active" and lic.is_paid:
            if lic.expiry_date and datetime.now() > lic.expiry_date:
                return jsonify({"valid": False, "message": "License expired"}), 200
            return jsonify({"valid": True}), 200
        else:
            return jsonify({"valid": False, "message": f"License status is {lic.status}"}), 200
    except Exception as e:
        logging.exception("Failed to verify license key")
