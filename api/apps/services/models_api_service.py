...
def _get_model_info(tenant_id: str, default_model: str, model_type: str, model_id: str = ""):
    ...
    if model_record:
        if not model_record.status == ActiveStatusEnum.ACTIVE.value:
            ...
            return None
        return {"model_id": model_id or model_record.id, "model_provider": provider_name, "model_instance": instance_name, "model_name": model_name, "model_type": model_type, "enable": True}
...
def list_tenant_default_models(tenant_id: str):
    ...
    for model_type, field_name in MODEL_TYPE_TO_FIELD.items():
        ...
        tenant_model_id = getattr(tenant, MODEL_TYPE_TO_TENANT_ID_FIELD.get(model_type, ""), None)
        model_info = None
            ...
            try:
                ...
                for gm in global_models:
                    if normalize_model_type(gm.model_type) == model_type:
                        ...
                        model_info = _get_model_info(tenant_id, cand_str, model_type, gm.id)
                        if model_info:
                            break
            except Exception:
                pass
...
def list_tenant_added_models(tenant_id: str, model_type_filter: str = None):
    ...
    try:
        ...
        for gm in global_models:
            ...
            if (gm_prov_lower, gm.model_name.lower()) not in existing_model_keys:
                has_key = bool(gm.api_key and gm.api_key.strip()) or (gm_prov_lower in active_provs)
                if not has_key:
                    continue
                ...
                added_models.append({
                    ...
                    "instance_id": "",
                    "instance_name": "default",
                    "rank": model_rank_map.get((gm_prov, gm.model_name), 500),
                })
    ...
    added_models.sort(key=lambda x: (factory_rank_mapping.get(x["provider_name"], -500), x["provider_name"], x["instance_name"], -x.get("rank", 500), x["name"]))
